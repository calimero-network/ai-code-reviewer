...
| No findings + all agents succeeded | `APPROVE` (if configured) |
| Any agent failed | `COMMENT` (never `APPROVE`, even if no findings) |
| Review infrastructure error | Comment with error, no action |

## Output Format (GitHub Markdown)

### Success Case (all agents succeeded)
...
### Incomplete Review (one or more agents failed)

When `failed_agents` is non-empty, the review renders:

```markdown
## 🤖 AI Code Review

⚠️ **Review Incomplete** — Agents failed: {failed_agent_names}

### 🔴 Critical Issues ({count})
[findings from agents that succeeded...]
...
⚠️ *Partial review: {N} of {total_agents} agents completed. See logs for failure details.*
...
**Key behaviors:**
- A "Review Incomplete" header names the failed agents.
- A partial-review trailer is appended even if other agents found real issues.
- The review action is `COMMENT`, never `APPROVE`, regardless of findings.
- When both failed agents and critical findings exist, `REQUEST_CHANGES` is used.
...
7. **Don't APPROVE when agents failed** - Even if findings are empty, a failed agent means `COMMENT`, never `APPROVE`
8. **Don't skip `all_agents_failed` check on LGTM fast-path** - Both CLI and webhook LGTM re-check paths must verify `not recheck_review.all_agents_failed` before posting the LGTM placeholder; a failed re-check falls through to a full review
